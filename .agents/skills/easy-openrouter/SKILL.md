...
- Combinable: `anthropic/Codex-sonnet-4:nitro:online`
...
**Example**: Find fastest provider for Codex-sonnet-4

```
https://openrouter.ai/anthropic/Codex-sonnet-4/providers?sort=throughput
```
...
```bash
# Test Codex
bash scripts/call_openrouter.sh --model "anthropic/Codex-sonnet-4" --prompt "Explain microservices architecture" --json > Codex.json

# Test GPT-4o
bash scripts/call_openrouter.sh --model "openai/gpt-4o" --prompt "Explain microservices architecture" --json > gpt4o.json

# Compare results
jq -s '[.[0], .[1]] | map({model, response_time_ms, cost})' Codex.json gpt4o.json
```
...
- **Quality-first**: Use `Codex-sonnet-4` or `gpt-4o`
